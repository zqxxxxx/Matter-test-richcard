...
func (ba *BotAPI) Route(r *wkhttp.WKHttp) {
	...
	// Internal service-to-service endpoints (X-Internal-Token, fail-closed).
	// See internal_send.go.
	internal := r.Group("/v1/internal/bot", ba.internalAuthMiddleware())
	{
		internal.POST("/sendMessage", ba.internalSendMessage)
		internal.GET("/groups", ba.internalBotGroups)
	}
